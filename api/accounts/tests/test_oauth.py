...
class OAuthTests(TestCase):
    ...
    def test_github_prod_flow_with_invite(self):
        ...
        self.assertTrue(
            OrgUser.objects.filter(org=self.org, user__email=email).exists()
        )
        inv.refresh_from_db()
...
class GoogleOAuthTests(TestCase):
    ...
    def test_google_callback_debug_email_shortcut(self):
        resp = self.client.post('/api/oauth/google/callback', data={
            'code': 'dummy',
    ...
    @override_settings(DEBUG=False, GOOGLE_CLIENT_ID='client', GOOGLE_CLIENT_SECRET='secret', OAUTH_REDIRECT_URI='http://localhost/cb')
