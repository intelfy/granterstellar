...
class RLSExtendedMatrixTests(TestCase):
	...
	def setUpTestData(cls):
		...
		set_guc(user_id=cls.admin.id)
		cls.file_admin = FileUpload.objects.create(owner=cls.admin, file=ContentFile(b"admin", name="admin.txt"), content_type="text/plain", size=5)
		set_guc(user_id=cls.member.id)
		cls.file_member = FileUpload.objects.create(owner=cls.member, file=ContentFile(b"member", name="member.txt"), content_type="text/plain", size=6)
